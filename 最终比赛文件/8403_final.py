...
xianzhitime = 42  # 驶过障碍的时间

# 击靶参数
fw_times = 700  # 击靶前进时间
fw_speed = 30  # 击靶前进速度
ssumyuzhi = 2.05e+07  # 画面数组最大值
flag_max = 2  # 数组判断最大值
MinRadius = 0  # 最小霍夫圆圈
MaxRadius = 250  # 最大霍夫圆圈
minyuanxinyuzhi = 210  # 最小圆心阈值
maxyuanxinyuzhi = 270  # 最大圆心阈值
red_yuzhi = 5  # 红色二值化阈值
...
def adjust():  # 击靶调整
    ...
    while True:
        if cap1.isOpened():
            ...
            time.sleep(1)
            robot.movement.move_forward(speed=fw_speed,  times=fw_times)
...
def auto_pilot():  # 自主前进程序
    with tf.Session(graph=inference_path) as sess:
        ...
        while True:
            ...
            if time_c < 7 or time_c > xianzhitime:  # 第一个转弯速度慢点
                forwardspeed = 15
            else:
                forwardspeed = forward#31
